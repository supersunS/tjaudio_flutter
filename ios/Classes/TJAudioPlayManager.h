...
//判断字符串是否为空
#define beNil(aString) (![aString isKindOfClass:[NSString class]]||[aString isKindOfClass:[NSNull class]]|| aString==nil||((NSString *)aString).length == 0||[aString stringByReplacingOccurrencesOfString:@" " withString:@""].length == 0)

//判断字符串为空并返回空字符串

#define returnBeNil(aString)\
({\
NSString *resultStr = beNil(aString) ? @"" : aString;\
(resultStr);\
})
