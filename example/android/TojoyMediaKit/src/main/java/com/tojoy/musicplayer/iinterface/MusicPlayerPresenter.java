package com.tojoy.musicplayer.iinterface;
...
public interface MusicPlayerPresenter {
    void setCurrentPageType(int type,long addTime);
    /**
     * 是否开启默认的通知栏交互，默认开启状态
     ...
     */
    void addPlayMusicToTop(AudioInfo audioInfo);
    ...
    /**
     * 只更新内部播放器的数据集合同步至内部播放器
     * @param audios 待播放列表
     */
    void updateMusicPlayerData(List<?> audios);

    /**
     * 将缓存数据替换给当前播放的数据
     */
    void copyCacheMusicList();
     ...
     * @return 音频对象
     */
    AudioInfo getCurrentPlayerMusic(int index);
    /**
     * 返回正在播放的音频对象
     * @return 音频对象
     */
    AudioInfo getCurrentPlayerMusic();
     ...
     * @param listener 实现监听器的对象
     */
    void addOnVedioPlayerEventListener(VedioPlayerEventListener listener);
    void removeVedioPlayerListener(VedioPlayerEventListener listener);
    /**
     * 清空监听器池子所有的监听器对象
     */
    ...
    void setVedioPlayPage(boolean isBossVedioPage);
