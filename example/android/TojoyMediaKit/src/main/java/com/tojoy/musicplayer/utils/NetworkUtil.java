...
public class NetworkUtil {
    ...
    public static boolean checkNet(Context context) {
        boolean isAvailable = isNetAvailable(context);
        if (!isAvailable) {
            try {
                Toast.makeText(context,context.getString(R.string.newwork_error_tip),Toast.LENGTH_LONG).show();
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
    ...
    /**
     * 判断有网络的情况下，当前网络状态是否是wifi，否则默认为移动流量链接
     * @return
     */
    public static boolean currentNetStatusIsWifi(Context context){
        boolean isWifi=true;
        boolean isAvailable = isNetAvailable(context);
        //网络有效，且不是wifi环境则返回false其他情况均视为移动网络流量链接
        if (isAvailable&&!isWifi(context)) {
            isWifi=false;
        }
        return isWifi;
    }
    ...
    public static String getNetworkTypeForLinkStr(Context context) {
        try {
            ConnectivityManager cm = (ConnectivityManager) context.getSystemService(Context.CONNECTIVITY_SERVICE);
            NetworkInfo ni = cm.getActiveNetworkInfo();
            if (ni != null) {
